...
SWITCH_DECLARE(void) switch_core_media_patch_sdp(switch_core_session_t *session)
{
	...
	while (p && *p) {
		...
		if (a_engine->local_sdp_ip && !strncmp("c=IN IP", p, 7)) {
			...
			p +=2;
			q +=2;
			memcpy(q, a_engine->adv_sdp_ip, strlen(a_engine->adv_sdp_ip));
			...
			memcpy(q, port_buf, strlen(port_buf));
			q += strlen(port_buf);
			...
			memcpy(q, vport_buf, strlen(vport_buf));
			q += strlen(vport_buf);
			...
			memcpy(q, tport_buf, strlen(tport_buf));
			q += strlen(tport_buf);
