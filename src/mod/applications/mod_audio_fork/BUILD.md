...
`Freeswitch.PTT.Minimal.2017.slnf` now includes `mod_audio_fork`, but the Visual Studio project expects these external dependencies to be available first:

- `LIBWEBSOCKETS_DIR` - root directory containing `include\libwebsockets.h` and the corresponding `.lib`
- `LIBWEBSOCKETS_LIB` - optional override for the import/static library name (default: `websockets.lib`)
- `BOOST_INCLUDEDIR` or `BOOST_ROOT` - include path for Boost headers (`boost/circular_buffer.hpp`)

Example:

```bat
set LIBWEBSOCKETS_DIR=C:\deps\libwebsockets
set LIBWEBSOCKETS_LIB=websockets_static.lib
set BOOST_INCLUDEDIR=C:\local\boost_1_86_0
build-ptt-minimal.cmd Release x64
```
